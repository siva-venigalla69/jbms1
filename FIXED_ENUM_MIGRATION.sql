-- FIXED ENUM MIGRATION - Resolves enum schema context issues
-- This script properly handles enum creation and usage in jbms_db.public schema

-- Step 1: Handle any aborted transactions
ROLLBACK;

-- Step 2: Drop and recreate the adjustment_type enum with explicit schema
BEGIN;

-- Drop the enum if it exists (this will also drop dependent objects)
DROP TYPE IF EXISTS jbms_db.public.adjustment_type CASCADE;
DROP TYPE IF EXISTS public.adjustment_type CASCADE;

-- Create the enum explicitly in the jbms_db.public schema
CREATE TYPE jbms_db.public.adjustment_type AS ENUM ('quantity_change', 'reason');

-- Set the search path to include our schema for easier table operations
SET search_path TO "jbms_db.public", public;

COMMIT;

-- Step 4: Main schema migration with correct enum context
BEGIN;
...
-- NOW create inventory_adjustments table - enum should now be accessible
DROP TABLE IF EXISTS jbms_db.public.inventory_adjustments CASCADE;
CREATE TABLE jbms_db.public.inventory_adjustments (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    inventory_id UUID NOT NULL REFERENCES jbms_db.public.inventory(id),
    adjustment_type jbms_db.public.adjustment_type NOT NULL,
...
-- Create views with proper schema handling
...
-- Step 5: Test that the enum and table creation worked
BEGIN;
...
-- Final verification
SELECT 'Schema migration completed successfully with fixed enum handling!' as final_message; 