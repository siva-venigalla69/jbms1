...
class LocalAPITester:
    ...
    def test_customer_endpoints(self):
        ...
        # Create customer
        test_customer_data = {
            "name": "Test Customer LocalAPI",
            "phone": "1234567890",
            "email": "test@localapi.com",
            "address": "Test Address",
            "gst_number": "TEST123456789"
        }
        
        try:
            response = self.make_request("POST", "/api/customers", json_data=test_customer_data)
    ...
    def test_order_endpoints(self):
        ...
        # Create order with correct MaterialType enum values
                ...
                "order_items": [
                    {
                        ...
                        "quantity": 5,
                        "unit_price": 250.00,
                        "customization_details": "Special design requirements"
                    },
                    {
                        "material_type": "dupatta",
                        "quantity": 3,
                        "unit_price": 150.00,
                        "customization_details": "Matching design"
                    }
                ]
            }
            
            try:
                response = self.make_request("POST", "/api/orders", json_data=test_order_data)
                success = response.status_code in [200, 201]
                if success:
                    order = response.json()
                    self.test_data['order_id'] = order.get('id')
                    if order.get('order_items'):
                        self.test_data['order_item_id'] = order['order_items'][0].get('id')
    ...
    def test_order_items(self):
        ...
        # Create order item (if separate endpoint exists)
        item_data = {
            "order_id": self.test_data['order_id'],
            "material_type": "voni",
            "quantity": 1,
            "unit_price": 300.00,
            "customization_details": "Traditional voni design"
        }
        ...
        try:
            ...
            if success:
                item = response.json()
                if not self.test_data.get('order_item_id'):
    ...
    def test_material_endpoints(self):
        ...
        # Record material in with correct MaterialType
        test_material_in_data = {
            "customer_id": "",  # Will be set after customer creation
            "material_type": "saree",  # Use lowercase to match database enum
            "quantity": 10,
            "unit": "pieces",
            "received_date": datetime.now().isoformat(),
            "notes": "Materials received for testing"
        }
        
        if self.test_data.get('customer_id'):
            test_material_in_data["customer_id"] = self.test_data['customer_id']
        
        try:
            response = self.make_request("POST", "/api/materials/in", json_data=test_material_in_data)
            success = response.status_code in [200, 201]
            if success:
    ...
    def test_challan_endpoints(self):
        ...
        if self.test_data.get('customer_id') and self.test_data.get('order_item_id'):
            challan_data = {
                "customer_id": self.test_data['customer_id'],
                "challan_date": datetime.now().isoformat(),
            ...
            self.log_test("Create Challan", False, f"Missing dependencies: customer_id={self.test_data.get('customer_id')}, order_item_id={self.test_data.get('order_item_id')}")
    ...
    def test_invoice_endpoints(self):
        ...
        if self.test_data.get('customer_id') and self.test_data.get('challan_id'):
            invoice_data = {
                ...
                "sgst_rate": 9.0,
                "igst_rate": 0.0,
                "challan_ids": [self.test_data['challan_id']]
            }
    ...
    def test_expense_endpoints(self):
        ...
        expense_data = {
            ...
            "amount": 500.00,
            "payment_method": "cash",
            "reference_number": f"REC{int(time.time())}",
    ...
    def run_all_tests(self):
        """Run all API tests"""
        print("=" * 80)
        print("🚀 COMPREHENSIVE LOCAL API TESTING SUITE - FIXED VERSION")
        print("=" * 80)
    ...
    def save_results(self):
        ...
        filename = f"local_api_test_results_fixed_{timestamp}.json"
...
def main():
    ...
    print("🧪 Starting Fixed Local API Testing...")
